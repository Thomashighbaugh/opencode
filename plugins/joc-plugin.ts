/**
 * OpenCode JOC (Joint Operations Center) Plugin
 * 
 * A TypeScript plugin for OpenCode that implements the hook system for multi-agent orchestration.
 * Named after the military field command post - a Joint Operations Center serves as the central
 * position from which commanders dispatch orders to combatants in the zone of operations.
 * Provides multi-agent orchestration capabilities including:
...
function hasActiveMode(directory: string, sessionId?: string): boolean {
  ...
  for (const file of MODE_STATE_FILES) {
    ...
    if (state?.active === true && !state.session_id) return true
  }
...
function loadSessionStats(): SessionStats {
  return readJsonFile<SessionStats>(SESSION_STATS_FILE) || { sessions: {} }
}

function saveSessionStats(stats: SessionStats): void {
  writeJsonFile(SESSION_STATS_FILE, stats)
}
...
${resolved.map(m => `- ${m.name}: Use /${m.name === 'ralph' ? 'ralph-loop' : m.name}`).join('\n')}`)
  ...
  hooks["experimental.chat.system.transform"] = async (input, output) => {
    ...
    const contextBlock = `<joc-plugin-context>\n${messages.join('\n\n')}\n</joc-plugin-context>`
