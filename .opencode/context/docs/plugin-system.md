...
> Complete reference for the Hubs hook system plugin — now split into focused modules under `plugins/hooks/`
...
The Hubs plugin system was refactored from a single 1711-line monolith (`hubs-plugin.ts`) into four focused modules under `plugins/hooks/`:
...
The plugin is now split into four modules under `plugins/hooks/`:

```typescript
// plugins/hooks/session.ts — Session lifecycle, stats, file tracking
...
// plugins/hooks/modes.ts — Mode state CRUD
...
// plugins/hooks/keywords.ts — Magic keyword detection
...
// plugins/hooks/hooks.ts — Plugin entry point (imports from the other three)
...
hooks["experimental.session.compacting"] = async (input, output) => {
  ...
  if (sessionId) {
    ...
    if (toolCalls > 75 || durationSec > 900 || subagentCalls > 5) {
      const artifact = {
        sessionId,
...
**Long session threshold** — artifact saved only when any of: >75 tool calls, >15 min duration, or >5 subagent invocations. See [State Management](../docs/state-management.md#compaction-artifacts) for full details.
...
- [Installation](../docs/installation.md) - Plugin installation
- [Execution Modes](../docs/execution-modes.md) - Mode state details
- [State Management](../docs/state-management.md) - State persistence
- [Tools](../docs/tools.md) - TypeScript tools