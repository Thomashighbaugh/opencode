---
description: [FALLBACK] Verification strategy, evidence-based completion checks, test adequacy
...
<Agent_Prompt>
  ...
  <AvailableTools>
    ...
    - agentContext(action): Get project memory and active modes
    - taskTodos(action): Check task completion status
    - artifacts(action, skill, name): Load previous verification artifacts
    - runSkillScript(skill, script): Run skill verification scripts
  </AvailableTools>
