...
`state_get_status`) may be registered as **deferred tools** by OpenCode. Before calling
...
SESSION_ID="${OPENCODE_SESSION_ID:-${OPENCODECODE_SESSION_ID:-}}"
REPO_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || { d="$PWD"; while [ "$d" != "/" ] && [ ! -d "$d/.opencode/state" ]; do d="$(dirname "$d")"; done; echo "$d"; })"
...
elif [ "$REPO_ROOT" != "/" ] && [ -d "$REPO_ROOT/.opencode/state" ]; then
  OMC_STATE="$REPO_ROOT/.opencode/state/state"
else
  echo "ERROR: Could not locate .opencode/state state directory" >&2
  exit 1
fi
...
- By default the command inspects the current session via `state_list_active` and `state_get_status`, navigating `.opencode/state/state/sessions/{sessionId}/…` to discover which mode is active.
- When a session id is provided or already known, that session-scoped path is authoritative. Legacy files in `.opencode/state/state/*.json` are consulted only as a compatibility fallback if the session id is missing or empty.
- Swarm is a shared SQLite/marker mode (`.opencode/state/state/swarm.db` / `.opencode/state/state/swarm-active.marker`) and is not session-scoped.
...
8. Team (OpenCode native)
9. OMC Teams (tmux CLI workers)
10. Plan Consensus (standalone)
11. Self-Improve (standalone — clear state, clean orphaned worktrees, preserve iteration_state for resume, set status: "user_stopped" in .opencode/state/self-improve/state/agent-settings.json)
...
1. `state_list_active` enumerates `.opencode/state/state/sessions/{sessionId}/…` to find every known session.
2. `state_clear` runs once per session to drop that session's files.
3. A global `state_clear` without `session_id` removes legacy files under `.opencode/state/state/*.json`, `.opencode/state/state/swarm*.db`, and compatibility artifacts (see list).
4. Team artifacts (`~/.config/opencode/teams/*/`, `~/.config/opencode/tasks/*/`, `.opencode/state/state/team-state.json`) are best-effort cleared as part of the legacy fallback.
...
- `.opencode/state/state/autopilot-state.json`
- `.opencode/state/state/ralph-state.json`
- `.opencode/state/state/ralph-plan-state.json`
- `.opencode/state/state/ralph-verification.json`
- `.opencode/state/state/ultrawork-state.json`
- `.opencode/state/state/ultraqa-state.json`
- `.opencode/state/state/swarm.db`
- `.opencode/state/state/swarm.db-wal`
- `.opencode/state/state/swarm.db-shm`
- `.opencode/state/state/swarm-active.marker`
- `.opencode/state/state/swarm-tasks.db`
- `.opencode/state/state/ultrapilot-state.json`
- `.opencode/state/state/ultrapilot-ownership.json`
- `.opencode/state/state/pipeline-state.json`
- `.opencode/state/state/omc-teams-state.json`
- `.opencode/state/state/plan-consensus.json`
- `.opencode/state/state/ralplan-state.json`
- `.opencode/state/state/boulder.json`
- `.opencode/state/state/hud-state.json`
- `.opencode/state/state/subagent-tracking.json`
- `.opencode/state/state/subagent-tracker.lock`
- `.opencode/state/state/rate-limit-daemon.pid`
- `.opencode/state/state/rate-limit-daemon.log`
- `.opencode/state/state/checkpoints/` (directory)
- `.opencode/state/state/sessions/` (empty directory cleanup after clearing sessions)
...
1. Call `state_list_active` to enumerate `.opencode/state/state/sessions/{sessionId}/…` and discover every active session.
...
3. If a `session_id` was supplied to `/cancel`, skip legacy fallback entirely and operate solely within that session path; otherwise, consult legacy files in `.opencode/state/state/*.json` only if the state tools report no active session. Swarm remains a shared SQLite/marker mode outside session scoping.
...
#### If Team Active (OpenCode native)

Teams are detected by checking for config files in `${OPENCODE_CONFIG_DIR:-~/.config/opencode}/teams/`:

```bash
# Check for active teams
TEAM_CONFIGS=$(find "${OPENCODE_CONFIG_DIR:-$HOME/.opencode}"/teams -name config.json -maxdepth 2 2>/dev/null)
```
...
```
For each team found in ${OPENCODE_CONFIG_DIR:-~/.config/opencode}/teams/:
...
**TeamDelete + Cleanup:**
```
  1. Call TeamDelete() — removes ~/.config/opencode/teams/{name}/ and ~/.config/opencode/tasks/{name}/
...
```bash
node "${OPENCODE_PLUGIN_ROOT}/scripts/cleanup-orphans.mjs" --team-name "{team_name}"
```
...
Team "{team_name}" cancelled:
  ...
  - Manual cleanup needed: yes/no
    Path: ~/.config/opencode/teams/{name}/ and ~/.config/opencode/tasks/{name}/
```

**Implementation note:** The cancel skill is executed by the LLM, not as a bash script. When you detect an active team:
1. Read `${OPENCODE_CONFIG_DIR:-~/.config/opencode}/teams/*/config.json` to find active teams
...
4. In force mode, iterate every active session, call `state_clear` per session, then run a global `state_clear` without `session_id` to drop legacy files (`.opencode/state/state/*.json`, compatibility artifacts) and report success. Swarm remains a shared SQLite/marker mode outside session scoping.
5. Team artifacts (`~/.config/opencode/teams/*/`, `~/.config/opencode/tasks/*/`, `.opencode/state/state/team-state.json`) remain best-effort cleanup items invoked during the legacy/global pass.
...
- **Local-only**: Clears state files in `.opencode/state/state/` directory
- **Resume-friendly**: Autopilot state is preserved for seamless resume
- **Team-aware**: Detects native OpenCode teams and performs graceful shutdown

## MCP Worker Cleanup

When cancelling modes that may have spawned MCP workers (team bridge daemons), the cancel skill should also:

1. **Check for active MCP workers**: Look for heartbeat files at `.opencode/state/state/team-bridge/{team}/*.heartbeat.json`
...
5. **Clean up shadow registry**: Remove `.opencode/state/state/team-mcp-workers.json`

### Force Clear Addition

When `--force` is used, also clean up:
```bash
rm -rf .opencode/state/state/team-bridge/       # Heartbeat files
rm -f .opencode/state/state/team-mcp-workers.json  # Shadow registry
