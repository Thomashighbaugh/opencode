...
All state lives under `.opencode/state/self-improve/`:

```
.opencode/state/self-improve/
...
OMC mode lifecycle: `.opencode/state/state/sessions/{sessionId}/self-improve-state.json`
...
| Planning | Hypothesis → structured plan | planner | opus |
| Architecture Review | 6-point plan review | architect | opus |
| Critic Review | Harness rule enforcement | critic | opus |
| Execution | Implement plan + run benchmark | executor | opus |
| Git Operations | Atomic merge/tag/PR | git-master | sonnet |
...
| File | Purpose |
|---|---|
| `.opencode/state/self-improve/config/settings.json` | User config: `number_of_agents`, `benchmark_command`, `benchmark_format`, `benchmark_direction`, `max_iterations`, `plateau_threshold`, `plateau_window`, `target_value`, `primary_metric`, `sealed_files`, `regression_threshold`, `circuit_breaker_threshold`, `target_branch`, `current_repo_url`, `fork_url`, `upstream_url` |
| `.opencode/state/self-improve/state/agent-settings.json` | Runtime: `iterations`, `best_score`, `plateau_consecutive_count`, `circuit_breaker_count`, `status`, `goal_slug` (derived: lowercase underscore from goal objective, persisted for cross-session consistency) |
| `.opencode/state/self-improve/state/iteration_state.json` | Per-iteration progress for resumability |
| `.opencode/state/self-improve/config/goal.md` | Improvement objective, target metric, scope |
| `.opencode/state/self-improve/config/harness.md` | Guardrail rules (H001, H002, H003) |
...
2. Create `.opencode/state/self-improve/` directory structure by copying from `templates/` in this skill directory.
3. Read `.opencode/state/self-improve/state/agent-settings.json`. Check `si_setting_goal`, `si_setting_benchmark`, `si_setting_harness`.
...
5. If goal not set → read `si-goal-clarifier.md` from this skill directory and run the 4-dimension Socratic interview directly in this context (Objective, Metric, Target, Scope). Write result to `.opencode/state/self-improve/config/goal.md`.
...
If state is cleared (cancel was invoked) OR status is `user_stopped`:
  a. Set `status: "user_stopped"` in `.opencode/state/self-improve/state/agent-settings.json`
...
Read `.opencode/state/self-improve/config/idea.md`. If non-empty, snapshot contents for planners. Clear after planners consume.
...
- Path to target repo
- Path to `.opencode/state/self-improve/config/goal.md`
- Path to `.opencode/state/self-improve/state/iteration_history/` (all prior records)
- Path to `.opencode/state/self-improve/state/research_briefs/` (prior briefs)
- Content of `data_contracts.md` Section 3 (Research Brief schema)

Expected output: research brief JSON → `.opencode/state/self-improve/state/research_briefs/round_{n}.json`

If researcher fails, proceed with history only.

### Step 5 — Plan

Spawn N `planner`(model=opus) agents in parallel (N = `number_of_agents` from settings).

Pass in each planner's prompt:
- Planner identity (planner_a, planner_b, planner_c...)
- Research brief path
- Iteration history path
- Harness rules from `.opencode/state/self-improve/config/harness.md`
...
Expected output: Plan Document JSON → `.opencode/state/self-improve/plans/round_{n}/plan_planner_{id}.json`

### Step 6 — Review

For each plan, **sequentially** (architect before critic):

**6a. Architecture Review**: Spawn `architect` with the plan + 6-point checklist:
...
**6b. Critic Review**: Spawn `critic` with the plan + harness rules:
...
For each approved plan, spawn `executor`(model=opus) in parallel.
...
4. **Ranked-candidate loop** — for each candidate in rank order (best first):
   ...
   b. **Merge** via `git-master`: `git merge experiment/round_{n}_executor_{id} --no-ff -m "Iteration {n}: {hypothesis} (score: {before} → {after})"`
...
8. **Write Merge Report** JSON to `.opencode/state/self-improve/state/merge_reports/round_{n}.json` (schema: data_contracts.md Section 9).

### Step 9 — Record & Visualize

1. Write iteration history to `.opencode/state/self-improve/state/iteration_history/round_{n}.json`
2. Update `.opencode/state/self-improve/state/agent-settings.json`:
...
2. Read `.opencode/state/self-improve/state/agent-settings.json`:
...
4. Read `.opencode/state/self-improve/state/iteration_state.json`:
