...
Diagnose and fix issues with an OpenCode Hubs self-managed global file installation at `~/.config/opencode/`.
...
PLUGIN_FILE="$GLOBAL_DIR/plugins/hubs-plugin.ts"
if [ -f "$PLUGIN_FILE" ]; then
  echo "  ✓ hubs-plugin.ts exists"
else
  echo "  ⚠ No hubs-plugin.ts — plugin hooks won't activate"
  WARN_COUNT=$((WARN_COUNT + 1))
fi
```
...
> Fixes applied. Re-run `/hubs-doctor` to verify, or `/init-project --force` for a full refresh.