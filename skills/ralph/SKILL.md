...
1. **PRD Setup** (first iteration only):
   a. Check if `prd.json` exists (in project root or `.opencode/state/`). If it already exists, read it and proceed to Step 2.
   b. If no `prd.json` exists, the system has auto-generated a scaffold. Read `.opencode/state/prd.json`.
