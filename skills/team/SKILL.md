...
User: "/team 3:executor fix all TypeScript errors"
      ...
      [TEAM ORCHESTRATOR (Lead)]
              ...
              +-- Completion
                      ...
                      -> rm .opencode/state/state/team-state.json
```
...
2. **Specialist agents complement executor agents.** Route analysis/review to architect/critic OpenCode agents and UI work to designer agents. Tmux CLI workers are one-shot and don't participate in team communication.
...
The lead writes handoffs to `.opencode/state/handoffs/<stage-name>.md`.
...
3. **On team cancellation, handoffs survive** in `.opencode/state/handoffs/` for session resume. They are not deleted by `TeamDelete`.
...
- **Resume:** restart from the last non-terminal stage using staged state + live task status. Read `.opencode/state/handoffs/` to recover stage transition context.
- **Cancel:** `/cancel` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then deletes team resources and clears/preserves Team state per policy. Handoff files in `.opencode/state/handoffs/` are preserved for potential resume.
...
{
  "team_name": "fix-ts-errors",
  "team_file_path": "~/.config/opencode/teams/fix-ts-errors/config.json",
...
4. **Delete team** -- Call `TeamDelete` to clean up:
   ...
   ```
5. **Clean OMC state** -- Remove `.opencode/state/state/team-state.json`
...
- `ollama_worker`: Emphasize strict TaskList/TaskUpdate/SendMessage loop and no orchestration commands.
- `ollama_worker`: Emphasize CLI API lifecycle (`omc team api ... --json`) and explicit failure ACKs with stderr.
- `ollama_worker`: Emphasize bounded file ownership and milestone ACKs after each completed sub-step.
...
```bash
node "${OPENCODE_PLUGIN_ROOT}/scripts/cleanup-orphans.mjs" --team-name fix-ts-errors
```
...
The team skill supports **hybrid execution** combining OpenCode agent teammates with external CLI workers (Codex CLI and Gemini CLI). Both types can make code changes -- they differ in capabilities and cost. These are standalone CLI tools, not MCP servers.
...
**Key difference from OpenCode teammates:**
- CLI workers operate via tmux, not OpenCode's tool system
...
|-----------|-----------|-----|
| Iterative multi-step work | OpenCode teammate | Needs tool-mediated iteration + team communication |
| Code review / security audit | CLI worker or specialist agent | Autonomous execution, good at structured analysis |
| Architecture analysis / planning | architect OpenCode agent | Strong analytical reasoning with codebase access |
| Refactoring (well-scoped) | CLI worker or executor agent | Autonomous execution, good at structured transforms |
| UI/frontend implementation | designer OpenCode agent | Design expertise, framework idioms |
| Large-scale documentation | writer OpenCode agent | Writing expertise + large context for consistency |
| Build/test iteration loops | OpenCode teammate | Needs Bash tool + iterative fix cycles |
| Tasks needing team coordination | OpenCode teammate | Needs SendMessage for status updates |
...
Task decomposition:
#1 [ollama_worker] Security review of current auth code -> output to .opencode/state/research/auth-security.md
#2 [ollama_worker] Refactor auth/login.ts and auth/session.ts (uses #1 findings)
#3 [ollama_worker:designer] Redesign auth UI components (login form, session indicator)
#4 [ollama_worker] Update auth tests + fix integration issues
#5 [ollama_worker] Final code review of all changes
```

The lead runs #1 (Codex security analysis), then #2 and #3 in parallel (Codex refactors backend, designer agent redesigns frontend), then #4 (OpenCode teammate handles test iteration), then #5 (Gemini final review).
...
This approach complements the existing `SendMessage`-based communication by providing a pull-based mechanism for MCP workers that cannot use OpenCode's team messaging tools.
...
1. Check `${OPENCODE_CONFIG_DIR:-~/.config/opencode}/teams/` for teams matching the task slug
...
|--------|--------------|----------------------|
| **Storage** | JSON files in `~/.config/opencode/teams/` and `~/.config/opencode/tasks/` | SQLite in `.opencode/state/state/swarm.db` |
...
| **Heartbeat** | Automatic idle notifications from OpenCode | Manual heartbeat table + polling |
...
**When to use Team over Swarm:** Always prefer `/team` for new work. It uses OpenCode's built-in infrastructure, requires no external dependencies, supports inter-agent communication, and has task dependency management.
...
If teammates are unresponsive, `TeamDelete` may fail. In that case, the cancel skill should wait briefly and retry, or inform the user to manually clean up `~/.config/opencode/teams/{team_name}/` and `~/.config/opencode/tasks/{team_name}/`.
...
- **Event log**: All team events are appended to `.opencode/state/state/team/{teamName}/events.jsonl`
- **Worker status files**: Workers write status to `.opencode/state/state/team/{teamName}/workers/{name}/status.json`
...
> **Note:** Team members do not have a hardcoded model default. Each teammate is a separate OpenCode session that inherits the user's configured model. Since teammates can spawn their own subagents, the session model acts as the orchestration layer while subagents can use any model tier.
...
1. `TeamDelete` handles all OpenCode state:
   - Removes `~/.config/opencode/teams/{team_name}/` (config)
   - Removes `~/.config/opencode/tasks/{team_name}/` (all task files + lock)
...
1. **Worktree creation**: Before spawning a worker, call `createWorkerWorktree(teamName, workerName, repoRoot)` to create an isolated worktree at `.opencode/state/worktrees/{team}/{worker}` with branch `omc-team/{teamName}/{workerName}`.
