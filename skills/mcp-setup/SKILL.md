...
Then construct and run the appropriate `opencode mcp add` command:
...
- Run `opencode mcp list` to verify server status
- Check server logs for errors

### API Key Issues
- Exa: Verify key at https://dashboard.exa.ai
- GitHub: Ensure token has required scopes (repo, read:org)
- Re-run `opencode mcp add` with correct credentials if needed
...
- Run `opencode mcp list` to confirm servers are active

### Removing or Updating a Server
- Remove: `opencode mcp remove <server-name>`
