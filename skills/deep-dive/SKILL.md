...
handoff: .opencode/state/specs/deep-dive-{slug}.md
---
...
Use **OpenCode built-in team mode** to run 3 parallel tracer lanes:
...
Save to `.opencode/state/specs/deep-dive-trace-{slug}.md`:
...
After saving:
- Persist `trace_path` in state: `state_write` with `state.trace_path = ".opencode/state/specs/deep-dive-trace-{slug}.md"`
...
- Save to `.opencode/state/specs/deep-dive-{slug}.md`
- Persist `spec_path` in state: `state_write` with `state.spec_path = ".opencode/state/specs/deep-dive-{slug}.md"`
...
- Use `Write` tool to save trace result and final spec to `.opencode/state/specs/`
...
- [ ] Phase 3 saves trace result to `.opencode/state/specs/deep-dive-trace-{slug}.md` with per-lane critical unknowns
...
- [ ] Final spec saved to `.opencode/state/specs/deep-dive-{slug}.md` in standard deep-interview format
...
Deep-dive's output (`.opencode/state/specs/deep-dive-{slug}.md`) feeds into the standard omc pipeline:

```
/deep-dive "problem"
  → Trace (3 parallel lanes) + Interview (Socratic Q&A)
  → Spec: .opencode/state/specs/deep-dive-{slug}.md

  → /omc-plan --consensus --direct (spec as input)
    → Planner/Architect/Critic consensus
    → Plan: .opencode/state/plans/ralplan-*.md
