...
3. **Options**: --branch, --base, --no-opencode, --no-tmux, etc.
...
8. **Launch OpenCode** (unless --no-opencode):
   ```bash
...
6. **Update registry, create tmux, launch OpenCode** (same as review)
...
4. **Create session, tmux, launch OpenCode** (same pattern)
