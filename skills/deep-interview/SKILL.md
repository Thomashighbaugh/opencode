...
2. **Write to file**: `.opencode/state/specs/deep-interview-{slug}.md`
...
- [ ] Spec file written to `.opencode/state/specs/deep-interview-{slug}.md`
...
If interrupted, run `/deep-interview` again. The skill reads state from `.opencode/state/state/deep-interview-state.json` and resumes from the last completed round.
...
/deep-interview "vague idea"
  ...
  → Spec written to .opencode/state/specs/deep-interview-{slug}.md
  ...
  → /omc-plan --consensus --direct (spec as input, skip interview)
    ...
    → Consensus plan written to .opencode/state/plans/
