...
Note: All `~/.config/opencode/...` paths in this guide respect `OPENCODE_CONFIG_DIR` when that environment variable is set.
...
1. Check if `~/.config/opencode/hud/omc-hud.mjs` exists
2. Check if `statusLine` is configured in `~/.config/opencode/settings.json`
3. If missing, create the HUD wrapper script and configure settings
4. Report status and prompt to restart OpenCode if changes were made

**IMPORTANT**: If the argument is `setup` OR if the HUD script doesn't exist at `~/.config/opencode/hud/omc-hud.mjs`, you MUST create the HUD files directly using the instructions below.

### Setup Instructions (Run These Commands)

**Step 1:** Check if setup is needed:
```bash
node -e "const p=require('path'),f=require('fs'),d=process.env.OPENCODE_CONFIG_DIR||p.join(require('os').homedir(),'.opencode');console.log(f.existsSync(p.join(d,'hud','omc-hud.mjs'))?'EXISTS':'MISSING')"
```
...
```bash
node -e "require('fs').mkdirSync(require('path').join(process.env.OPENCODE_CONFIG_DIR||require('path').join(require('os').homedir(),'.opencode'),'hud'),{recursive:true})"
```

Then, use the Write tool to create `${OPENCODE_CONFIG_DIR:-~/.config/opencode}/hud/omc-hud.mjs` with this exact content:
...
async function main() {
  ...
  if (pluginCacheDir && existsSync(pluginCacheDir)) {
    ...
    console.log("[OMC HUD] Plugin cache found but no built versions. Run: /omc-setup");
  } else {
    // No plugin installation found at all
    console.log("[OMC HUD] Plugin not installed. Run: /omc-setup");
  }
}
...
```bash
node -e "if(process.platform==='win32'){console.log('Skipped (Windows)')}else{require('fs').chmodSync(require('path').join(process.env.OPENCODE_CONFIG_DIR||require('path').join(require('os').homedir(),'.opencode'),'hud','omc-hud.mjs'),0o755);console.log('Done')}"
```

**Step 4:** Update settings.json to use the HUD:

Read `${OPENCODE_CONFIG_DIR:-~/.config/opencode}/settings.json`, then update/add the `statusLine` field.
...
```bash
node -e "const p=require('path').join(require('os').homedir(),'.opencode','hud','omc-hud.mjs').split(require('path').sep).join('/');console.log(JSON.stringify(p))"
```

**IMPORTANT:** The command path MUST use forward slashes on all platforms. OpenCode executes statusLine commands via bash, which interprets backslashes as escape characters and breaks the path.
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node ${OPENCODE_CONFIG_DIR:-$HOME/.opencode}/hud/omc-hud.mjs"
  }
}
```
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node C:/Users/username/.opencode/hud/omc-hud.mjs"
  }
}
```
...
```bash
node -e "const p=require('path'),f=require('fs'),d=process.env.OPENCODE_CONFIG_DIR||p.join(require('os').homedir(),'.opencode'),t=p.join(d,'hud','omc-hud.mjs');try{if(f.existsSync(t)){f.unlinkSync(t);console.log('Removed legacy script')}else{console.log('No legacy script found')}}catch{}"
```

**Step 6:** Tell the user to restart OpenCode for changes to take effect.
...
HUD config is stored in `~/.config/opencode/settings.json` under the `omcHud` key (or your custom config directory if `OPENCODE_CONFIG_DIR` is set).

Legacy config location (deprecated): `~/.config/opencode/.opencode/state/hud-config.json`
...
2. Restart OpenCode after setup completes
3. If still not working, run `/omc-doctor` for full diagnostics

**Legacy string format migration:** Older OMC versions wrote `statusLine` as a plain string (e.g., `"~/.config/opencode/hud/omc-hud.mjs"`). Modern OpenCode (v2.1+) requires an object format. Running the installer or `/hud setup` will auto-migrate legacy strings to the correct object format:
```json
{
  "statusLine": {
    "type": "command",
...
Manual verification:
- HUD script: `~/.config/opencode/hud/omc-hud.mjs`
- Settings: `~/.config/opencode/settings.json` should have `statusLine` configured as an object with `type` and `command` fields
