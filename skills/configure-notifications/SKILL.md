...
This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with AskUserQuestion. Write the result to `${OPENCODE_CONFIG_DIR:-~/.config/opencode}/.omc-config.json`.

### Step 1: Detect Existing Configuration

```bash
CONFIG_FILE="${OPENCODE_CONFIG_DIR:-$HOME/.config/opencode}/.omc-config.json"
...
1. **Session end (Recommended)** - When an OpenCode session finishes
2. **Input needed** - When OpenCode is waiting for your response (great for long-running tasks)
...
Config saved to: ~/.config/opencode/.omc-config.json
...
2. **OpenCode** - Display as "OpenCode"
...
Check if `${OPENCODE_CONFIG_DIR:-~/.config/opencode}/omc_config.hook.json` exists. If it does, show the current configuration. If not, explain what it does.
...
Config file: ~/.config/opencode/omc_config.hook.json
```
...
**Options:**
1. **session-end** - When an OpenCode session finishes (most common)
2. **ask-user-question** - When OpenCode is waiting for input
3. **session-idle** - When OpenCode finishes and waits for input
...
Read or create `${OPENCODE_CONFIG_DIR:-~/.config/opencode}/omc_config.hook.json` and merge the new settings:
...
If `~/.config/opencode/omc_config.openclaw.json` exists, detect and offer migration:

**Step 1: Detect Legacy Config**
```bash
LEGACY_CONFIG="${OPENCODE_CONFIG_DIR:-$HOME/.config/opencode}/omc_config.openclaw.json"
...
| `{{tmuxSession}}` | tmux session name | `opencode:my-project` |
